...
SYSTEM_PROMPT = (
    ...
    + "상담매뉴얼 근거(경로가 '상담 > …'이고 본문이 Q./A. 형식)로 답할 때는 검증된 정답이므로 "
    ...
    "상담사례 근거(경로가 '상담사례 > …')는 실제 상담 통화에서 정제한 참고 사례다 — "
    "화면·업무·상담 근거와 함께 있으면 그쪽을 우선하고 사례는 '실제 문의 사례로는…'처럼 "
    "보조로만 인용한다. 상담사례 근거만 있으면 답하되 매뉴얼로 검증된 내용이 아님을 밝힌다."
)
...
def related_questions(q: str, hits: list[dict],
    ...
    scope가 걸린 답변이면 그 범위에서 답변 가능한 질문만 후보로 쓴다 — 부문
    버킷이 매뉴얼을 가로질러 묶이므로(예: '계좌' = 화면>계좌 + 상담사례>계좌)
    범위 밖 질문이 추천되면 클릭 시 근거 0건 막다른 골목이 된다."""
