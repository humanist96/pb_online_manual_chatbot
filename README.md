...
[![LLM](https://img.shields.io/badge/LLM-Ollama_|_Extractive-e4670a)](#)
...
- 🎯 **검증된 질문 추천** — 매뉴얼 Q&A에서 **자기-검색 검증을 통과한 질문뱅크**만 말풍선으로 추천(첫 화면 예상 질문·답변 후 관련 질문·스코프 연동), 추천 클릭의 게이트 통과율 실측 100%
- 🔢 **화면번호 중심 표기** — 근거 카드·범위 셀렉터가 단말 입력용 **화면번호**(예: 2150)를 주인공으로 원클릭 복사, 내부 문서코드는 비노출(LLM 답변에도 화면번호 주입)
...
**🌐 온라인 데모**: [pb-manual-desk-demo.vercel.app](https://pb-manual-desk-demo.vercel.app) — **접근키 제한 운영**(사내 담당자 문의). 스코프 칩·모호성 배너·근거 지도·질문 추천 말풍선·인앱 튜토리얼·피드백 게시판을 브라우저에서 사용해 볼 수 있습니다. 합성 데이터 공개 모드(`PUBLIC_DEMO`, 832청크)도 지원합니다.
...
| ④ | **무관 질의 차단** | "오늘 점심 뭐 먹지" | 게이트 차단(best 0.50 < τ) — **LLM 미호출**, "매뉴얼에서 확인되지 않습니다" |
| ⑤ | **교차 오염 감지** | "수수료 조회" (전 부문) | 근거가 현물결제·감사·출납에 분산 → **모호성 배너**로 부문 원클릭 확정 (동음이의 20문항 중 75% 감지) |
...
- **대화 경험** — 질문 이력(localStorage, 오늘/어제 그룹) · **검증된 질문뱅크 말풍선**(첫 화면 예상 질문 + "다른 질문 보기" 셔플 + 답변마다 근거 기반 관련 질문) · 답변 복사/재생성 · **화면번호 클릭 복사**(단말 입력용).
- **온보딩**(온라인 데모) — 최초 방문 환영 가이드 + 헤더 `?` 도움말 + **인앱 스포트라이트 튜토리얼**: 실제 화면 요소를 단계별 하이라이트하고 예시 질문을 자동 시연(데스크톱).
...
```
┌──────────────────── 오프라인 색인 (매뉴얼 변경 시) ────────────────────┐
│  crawl_toc.py ─▶ crawl.py ─▶ parse.py ─┬─▶ to_xlsx.py ─▶ 화면별 XLSX  │
│  (TOC 재귀 →     (HTML       (HTML→구조 │                              │
│   부문 매니페스트) 수집)       화 트리)   └─▶ to_chunks.py ─▶ chunks.jsonl│
│                              ([부문] 경로 보존 청크)  │               │
│                                                 ▼                     │
│                                        build_index.py                 │
│                                 (임베딩 + BM25 → data/index/)          │
...
│   · 근거 지도 + 근거 카드    ├─ /api/answer  검색→LLM(ollama|추출)│
...
| ②′ 도식 | `extract_pm_images.py` | 이미지 → `pm_image_text.json` | 업무매뉴얼의 흐름도·표 이미지를 **OCR(EasyOCR, 로컬)+VLM**으로 텍스트화. VLM 모델·서빙 방식은 승인 전 확정하지 않으며 배포하지 않음 — sha1 증분 캐시·샤드 병렬 |
...
`/000/PM/` — 계좌관리·출납관리 등 10부문)의 이원 구조이고, 온라인 데모에는
**상담매뉴얼**(고객지원센터 실제 상담 Q&A, 19개 업무)이 추가된 삼원 구조입니다.
모든 매뉴얼을 하나의 인덱스로 통합하되 `sector_path` 루트에 매뉴얼 레벨을 두어
(`화면 › 계좌 › …`, `업무 › 계좌관리 › …`, `상담 › …`) 스코프 셀렉터 1열에서
**매뉴얼을 먼저 선택**할 수 있고, "계좌 개설"처럼 여러 매뉴얼에 걸치는 질문은
배너가 **"화면 기준? 업무 기준?"** 을 먼저 묻습니다(`scope=업무` 지정 시 다른
매뉴얼 근거 완전 배제). 업무매뉴얼 특유의 흐름도/표 이미지는 로컬 OCR로
텍스트화되어 "계좌개설 절차" 같은 질문에 단계·TR코드로 답합니다(근거 카드에
`업무` 뱃지 + 원본 이미지 마커).
...
bash deploy/run.sh         # ③ 실행 → http://127.0.0.1:8000
```
...
docker compose up -d        # → http://127.0.0.1:8000 (호스트 loopback)
```
...
사내 배포와 별개로 무료 스택으로 운영하는 온라인 데모:
**Vercel Hobby**(정적+Python 함수) + **Upstash Vector**(하이브리드: 내장 임베딩+BM25, 검색 ~200ms)
+ **OpenAI**(명시적 opt-in, Redis 비용 가드가 없으면 추출형 강등). **이중 운영 모드**: 기본은
접근키 게이트 운영(익명 접근 불가 — 키 미설정 시 전면 차단), `PUBLIC_DEMO=true`일 때만 sha256
고정 **합성 데이터셋** 전용으로 익명 공개(비합성 산출물이면 fail-closed). 질문 추천 말풍선·
사용자 피드백 게시판·온보딩 인앱 튜토리얼 포함. 실제 배포·데이터 반출은
[`deploy/online/README.md`](deploy/online/README.md)의 승인 절차를 따른다.
...
| `LLM_BACKEND` | `none` | `none`(추출형) · `ollama`(명시적 opt-in). webapp은 `auto`/도구 backend를 허용하지 않음 |
| `LLM_MODEL` | `qwen2.5:7b-instruct` | Ollama 모델. Terra는 정확한 식별자·서빙 방식 승인 전 반영하지 않음 |
...
| `HOST` / `PORT` | `127.0.0.1` / `8000` | 기본 loopback 바인딩. 외부 공개는 인증 reverse proxy와 명시 승인 필요 |
...
| LLM | 로컬 **Ollama**(명시적 opt-in) / 추출-합성 폴백 |
...
**Q. LLM 없이도 답하나요?**
A. 네. 기본 `LLM_BACKEND=none`에서 **추출-합성 폴백**이 상위 근거를 출처와 함께 제시합니다. Ollama는 명시적 opt-in일 때만 사용합니다.
...
~~로컬 **리랭커** + 관련도 게이트~~ ✅ → ~~검증된 질문 추천 + 적중률 계측~~ ✅ → ~~사용자 피드백 수집~~ ✅
→ ~~온보딩·인앱 튜토리얼~~ ✅(온라인) → 답변 **스트리밍**(로컬) → 멀티턴 문맥 → 인용 무결성 검증 → 동의어 확장.
상세 로드맵은 [`품질사용성_고도화_기획.md`](품질사용성_고도화_기획.md), 설계 배경은 [`기획.md`](기획.md) 참조.
